...
def validate(request, validation_data):
    ...
    if request.method != 'GET':
        return (405, 'Payload validation failed: Wrong method, GET expected, got %s.' % request.method, [])
    ...
    if message and message == valid_message:
        return (200, 'Payload validated. Message: %s' % message, [{'message': message}])

    else:
        return (403, 'Payload validation failed. Message: %s' % message, [])
